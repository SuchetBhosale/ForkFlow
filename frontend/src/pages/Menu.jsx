...
function Menu() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await axios.get("http://localhost:5000/api/menuItem", { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleOrder = async () => {
    const token = localStorage.getItem("token");
    try {
      await axios.post("http://localhost:5000/api/orders", { items: cart, totalPrice }, { headers: { Authorization: `Bearer ${token}` } });
      setCart([]);
      navigate("/orders");
