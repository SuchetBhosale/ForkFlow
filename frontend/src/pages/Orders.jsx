...
function Orders() {
  const socket = io(`https://forkflow-backend.onrender.com`);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.get(`https://forkflow-backend.onrender.com/api/orders/allOrder`, { headers: { Authorization: `Bearer ${token}` } });
