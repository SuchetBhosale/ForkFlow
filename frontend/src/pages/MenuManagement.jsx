...
function MenuManagement() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        await axios.put(`http://localhost:5000/api/menuItem/${editId}`, { name, price: Number(price), category }, { headers: { Authorization: `Bearer ${token}` } });
      } else {
        await axios.post("http://localhost:5000/api/menuItem/menu", { name, price: Number(price), category }, { headers: { Authorization: `Bearer ${token}` } });
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/menuItem/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      fetchMenuItems();
