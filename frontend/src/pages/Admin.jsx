...
function Admin() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const res = await axios.get(`https://forkflow-backend.onrender.com/api/orders/allOrder`, {
  ...
  const onDragEnd = async (result) => {
    ...
    const token = localStorage.getItem("token");
    await axios.patch(`https://forkflow-backend.onrender.com/api/orders/updateStatus/${id}`, { status }, {
      headers: { Authorization: `Bearer ${token}` },
    });
  };
